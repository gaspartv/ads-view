...
import { uploadCategoryImage, deleteCategoryImage, reorderCategoryImages } from "@/app/actions/category";
...
export function ImageUploadModal({ isOpen, onClose, category }: ImageUploadModalProps) {
  ...
  useEffect(() => {
    if (category?.Images) {
      setLocalImages(category.Images);
    } else {
      setLocalImages([]);
    }
  }, [category?.Images]);
  ...
  const handleDelete = async (imageId: string) => {
    if (category?.Images && category.Images.length <= 1) {
      toast.error("A categoria precisa ter pelo menos 1 imagem.");
      return;
    }

    startTransition(async () => {
      const result = await deleteCategoryImage(category.id, imageId);
      if (result.success) {
  ...
  const handleDrop = async (e: React.DragEvent, dropIndex: number) => {
    ...
    startTransition(async () => {
      const result = await reorderCategoryImages(category.id, imageIds);
      if (result.success) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="md:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Gerenciar Imagens - {category?.name}</DialogTitle>
        </DialogHeader>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {localImages.map((img: any, index: number) => (
                ...
                >
                  ...
                  <img src={img.url} alt="Category" className="object-cover w-full h-full" />
