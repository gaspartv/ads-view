...
import { uploadProductImage, deleteProductImage, reorderProductImages } from "@/app/actions/product";
...
export function ImageUploadModal({ isOpen, onClose, product }: ImageUploadModalProps) {
  ...
  const handleDelete = async (imageId: string) => {
    if (product?.Images && product.Images.length <= 1) {
      toast.error("O produto precisa ter pelo menos 1 imagem.");
      return;
    }

    startTransition(async () => {
      const result = await deleteProductImage(product.id, imageId);
      if (result.success) {
  ...
  const handleDrop = async (e: React.DragEvent, dropIndex: number) => {
    ...
    startTransition(async () => {
      const result = await reorderProductImages(product.id, imageIds);
      if (result.success) {
